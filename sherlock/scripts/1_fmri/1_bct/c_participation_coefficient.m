...
% Consensus: use consensus approach (Dwyer et al., 2014)
% For each iteration, create a co-classification matrix D, where 1 = two
% nodes are in the same module, 0 = otherwise

% Average within network using Yeo atlas

clc; clear all;

dirs.binarized = '../../data/1_fmri/4_binarized';
dirs.pc = '../../data/1_fmri/5_metrics';
...
for s = 1:nSub
    ...
    P_visual = NaN(50,1);
    P_motor = NaN(50,1);
    P_attention = NaN(50,1);
    P_salience = NaN(50,1);
    P_limbic = NaN(50,1);
    P_control = NaN(50,1);
    P_default = NaN(50,1);
    P_subcortex = NaN(50,1);
    P_average = NaN(50,1);
    Mod_average = NaN(50, 1);
    ...
    for ev = 1:50
        ...
        subj_path = fullfile(dirs.binarized, sprintf('threshold_%s', threshold), ...
        ...
        % Identify number of modules per event
        numModules = numel(unique(most_common_module_mat));
        numModules_ev(ev,1)
        modules_per_event = [modules_per_event; numModules];
